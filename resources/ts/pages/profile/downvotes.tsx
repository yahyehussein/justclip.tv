...
const Downvotes = ({
  ...
  const [clips, setClips] = useRemember<Pagination<ClipType>>(
    {
      data: [],
      next_page_url: `/json/clip?page=1&vote=downvotes${sortBy}`,
      to: 10,
    },
    "clips"
  );
  ...
  const onSort = (sortBy: string) => {
    ...
    axios.get(`/json/clip?page=1&vote=downvotes${sortBy}`).then(({ data }) => {
      setClips(data);
      setEmpty(!!!data.from);
    });
  };
...
Downvotes.layout = (page: JSX.Element) => <Layout>{page}</Layout>;
